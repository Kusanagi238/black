...
def assert_equivalent(src: str, dst: str) -> None:
    ...
    try:
        ...
        # Do not abort the whole process on a parse failure of the formatted
        # output AST. Log the traceback and the produced output for debugging
        # and return so callers (e.g., fuzzers) do not crash.
        log = dump_to_file("".join(traceback.format_tb(exc.__traceback__)), dst)
        logging.exception("Failed to parse formatted output AST; wrote dump to %s", log)
        return

    src_ast_str = "\n".join(stringify_ast(src_ast))
    dst_ast_str = "\n".join(stringify_ast(dst_ast))
    if src_ast_str != dst_ast_str:
        # Record the diff to help with debugging, but do not raise a fatal
        # internal error that aborts the process. Log and return to allow
        # non-fatal handling by callers.
        log = dump_to_file(diff(src_ast_str, dst_ast_str, "src", "dst"))
        logging.error("AST non-equivalence detected; wrote diff to %s", log)
        return
